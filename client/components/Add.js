/*
The Add component renders a button. A modal dialog pops up when that button is pressed. Inside the modal dialog, we have some input fields and a submit button. 
The input fields are used to fill in the details of the new expense. Pressing the submit button triggers the insertNewExpense() function which sends the 
information to the server via the /insert route using Axios request. The server stores the information in the MongoDB.
